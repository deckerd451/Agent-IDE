...
test('repository-aware validation separates Xcode target gaps from npm script gaps before ranking', () => {
  const validation = `# Validation
...
- Repository type: Xcode/iOS
- Primary build system: Xcode
- Target validation status: Partial
...
- Run and document \`xcodebuild -list\` for the primary Xcode project or workspace.
- Run and document an Xcode-native \`xcodebuild build\` result or the local environment reason it was skipped.
- Run and document \`xcodebuild test\` when a test scheme and simulator/device destination are available.
`;
  const { candidates } = chooseNextImprovementWithCandidates({
    ...healthyArgs,
    validation,
...
test('repository-aware validation keeps npm gaps for primary Node repositories', () => {
  const validation = `# Validation

## Target Repository Validation Summary
- Repository type: Node/JavaScript
- Primary build system: npm/package.json
- Target validation status: Partial
...
- No safe npm test script was detected; automated behavioral coverage is unknown.
`;
  const { candidates } = chooseNextImprovementWithCandidates({ ...healthyArgs, validation });
  assert.ok(candidates.some((candidate) => /npm run build/i.test(`${candidate.title} ${candidate.evidence}`)));
  assert.ok(candidates.some((candidate) => /npm test|npm test script/i.test(`${candidate.title} ${candidate.evidence}`)));
});
