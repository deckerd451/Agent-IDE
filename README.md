...
The Control Plane also generates `.ai/next-improvement-prompt.md`, a deterministic next-improvement prompt for exactly one highest-leverage issue. Each selected issue is classified by actionability: `code-fixable`, `manual`, or `validation-experiment`. Selection prefers code-fixable issues over manual issues unless no code-fixable issue exists or the manual issue is critical; if only manual repository intent gaps remain, the recommended title is **Complete Manual Repository Intent Notes** and the Builder Prompt clearly warns that it is a manual product-owner task. If no code-fixable or manual issues remain, it recommends **Run AI Handoff Validation**. The **Recommended Implementation Prompt** card shows the prompt title, actionability, source risk or recommendation, reason, and one-click **Copy Builder Prompt** / **View Prompt** actions. The existing **Generate Builder Prompt** action copies this generated artifact instead of a generic prompt.
...
- `npm run next:improvement` for generating `.ai/next-improvement-prompt.md`, the single highest-leverage Codex-ready builder prompt shown in the Control Plane.
