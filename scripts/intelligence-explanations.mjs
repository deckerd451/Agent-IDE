...
export function explainEvidenceSynthesis(evidenceSynthesis = {}) {
  return {
    ...
    rule: 'Missing canonical fields may display deterministic suggestions only when exact repository-local wording is found outside .ai/goals.md; confidence comes from independent evidence groups, not generated artifact counts.',
    ...
    fields: Object.fromEntries(Object.entries(evidenceSynthesis.fields ?? {}).map(([key, field]) => [key, {
      ...
      state: field.state,
      independentEvidence: field.confidenceCalculation?.independentGroups ?? [],
      generatedConfirmations: field.confidenceCalculation?.generatedConfirmations ?? [],
      confidenceCalculation: field.confidenceCalculation,
      evidence: field.evidence ?? [],
