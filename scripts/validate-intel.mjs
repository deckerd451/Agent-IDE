...
function formatKnownGaps(scripts, commands, xcodeProjects, repositoryProfile) {
  const commandNames = new Set(commands.map(({ scriptName }) => scriptName));
  const gaps = [];

  const npmIsPrimary = repositoryProfile.packageJsonIsPrimary;
  const xcodeIsPrimary = repositoryProfile.primaryBuildSystem === 'Xcode';
  if (npmIsPrimary) {
    if (!commandNames.has('build')) gaps.push('No `npm run build` script was detected, so production build validation could not run.');
    if (!['test', 'test:unit', 'test:ci'].some((name) => commandNames.has(name))) gaps.push('No safe npm test script was detected; automated behavioral coverage is unknown.');
    if (!commandNames.has('lint')) gaps.push('No `npm run lint` script was detected; style/static lint coverage is unknown.');
    if (!['typecheck', 'type:check', 'check'].some((name) => commandNames.has(name)) && commandNames.has('build')) gaps.push('No standalone typecheck script was detected; type validation is covered only insofar as the build runs it.');
    if (Object.keys(scripts).length === 0) gaps.push('No package scripts were detected in `package.json`.');
  }
  if (xcodeIsPrimary) {
    gaps.push('Run and document `xcodebuild -list` for the primary Xcode project or workspace.');
    gaps.push('Run and document an Xcode-native `xcodebuild build` result or the local environment reason it was skipped.');
    gaps.push('Run and document `xcodebuild test` when a test scheme and simulator/device destination are available.');
    if (repositoryProfile.hasSwiftPackage) gaps.push('Run and document `swift test` or Swift package coverage if Package.swift is part of the repository.');
    gaps.push('Run SwiftLint only when a SwiftLint configuration or dependency is present.');
  } else if (xcodeProjects.projects.length > 0 || xcodeProjects.workspaces.length > 0) {
...
async function main() {
  ...
  const repositoryProfile = detectRepositoryProfile({ files: [...xcodeProjects.projects, ...xcodeProjects.workspaces, ...(await pathExists(packageJsonPath) ? ['package.json'] : [])], packageJson });
  ...
  const content = [
    ...
    '- Validation artifact generated by Agent IDE.',
    '',
    '## Target Repository Validation Summary',
    `- Repository type: ${repositoryProfile.repositoryType}`,
    `- Primary build system: ${repositoryProfile.primaryBuildSystem}`,
    `- Target validation status: ${overallStatus}`,
    '- Known target validation gaps are listed in ## Known Gaps.',
    '',
    ...
    '',
    '## Known Gaps',
    formatKnownGaps(scripts, commands, xcodeProjects, repositoryProfile),
    '',
    manualNotes,
  ].join('\n');
