...
export async function computeQualitySnapshot(repositoryPath, docs, previousQuality = null, previousHistory = []) {
  ...
  const focusValues = [mdSection(docs['goals.md'] ?? '', 'Current Focus'), mdSection(docs['strategy.md'] ?? '', 'Current Focus'), mdSection(docs['architecture.md'] ?? '', 'Current Focus')].filter(present).map((v) => firstLine(v, ''));
  const missingGeneratedFields = [];
  if (present(mdSection(docs['goals.md'] ?? '', 'Current Focus')) && !present(mdSection(docs['strategy.md'] ?? '', 'Current Focus'))) missingGeneratedFields.push('strategy.md: Current Focus');
